  ...
  <body>
    ...
    <div class="container">
      ...
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">후기 게시판</li>
      </ol>
			<img src="../images/about06.jpg" height="400px" width="1100px"><br><br>

			<center>
				<table border="0"cellspacing="2" cellpadding="4" class="table">
					<tr>
					...
					<input type="button" class="btn btn-primary" name="button" value="목록" onclick="location.href='list.php?page=<?=$page?>'"/>
					&nbsp;
	<?
		if($userid==$item_id || $userlevel==1 || $userid=="admin")
		{
	?>
					<input type="button" name="button" value="수정" class="btn btn-primary"
						onclick="location.href='modify_form.php?num=<?=$num?>&page=<?=$page?>'"/>&nbsp;

					<button  class="btn btn-primary" onclick="javascript:del('delete.php?num=<?=$num?>')">삭제</button>&nbsp;
...
<?if($userid){?>
	...
	<?
	    ...
	    $sql = "select * from reviewreply where linkno=$num";
		$result = mysql_query($sql, $connect);
